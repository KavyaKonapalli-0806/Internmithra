...
def normalize_edu_score(user_edu: str, intern_edu: str) -> float:
    levels = ["10th", "12th", "Diploma", "UG", "PG", "PhD"]
    try:
        u = levels.index(user_edu)
        v = levels.index(intern_edu)
    except ValueError:
        return 0.0
    diff = abs(u - v)
    return max(0.0, 1.0 - (diff / (len(levels) - 1)))
...
def score_internship(user_profile: Dict, intern: Dict, weights: Dict = WEIGHTS) -> Dict:
    ...
    loc_s = location_score(user_profile.get("state", ""), intern.get("state", ""), user_profile.get("remote", False), intern.get("remote", "no"))
...
def recommend(user_profile: Dict, internships: List[Dict], top_n: int = 5) -> List[Dict]:
    ...
    for intern in internships:
        ...
        intern_copy["skills"] = [s.strip() for s in str(intern.get("skills", "")).split(";") if s.strip()]
